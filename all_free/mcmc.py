...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    x_main_in, y_main_in, x_sub_in, y_sub_in, rs_main_in, rc_sub_in, alpha_main_in, beta_main_in, \
    gamma_main_in, beta_sub_in, A_main_in, A_sub_in = initial_params.init_cond_cls('gnfw_circ+beta_circ')
    ...
    out_file.write('alpha_main         %.4e       %.4e    \n' % (alpha_main_in[0], alpha_main_in[1]))
    out_file.write('gamma_main         %.4e      %.4e    \n' % (gamma_main_in[0], gamma_main_in[1]) )
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    alpha_main_first=np.random.uniform(alpha_main_in[0], alpha_main_in[1], Nwalkers)
    beta_main_first=np.random.uniform(beta_main_in[0], beta_main_in[1], Nwalkers)
    gamma_main_first=np.random.uniform(gamma_main_in[0], gamma_main_in[1], Nwalkers)
    ...
    init_sample = np.array ([x_main_first, y_main_first, x_sub_first, y_sub_first, rs_main_first, rc_sub_first, \
                           alpha_main_first, beta_main_first, gamma_main_first, beta_sub_first, A_main_first, A_sub_first, a_bkg_first]).T  #
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    autocorr_alpha_main=np.empty(Nsamples)
    autocorr_beta_main=np.empty(Nsamples)
    autocorr_gamma_main=np.empty(Nsamples)
...
with (Pool(ncores)) as pool:
    ...
    for k in range(N_check):
        ...
        if fit_method == 'gnfw_circ+beta_circ':
            ...
            autocorr_x_sub[index]=tau[2]
            autocorr_y_sub[index]=tau[3]
            autocorr_rs_main[index]=tau[4]
            autocorr_rc_sub[index]=tau[5]
            autocorr_alpha_main[index]=tau[6]
            autocorr_beta_main[index]=tau[7]
            autocorr_gamma_main[index]=tau[8]
            autocorr_beta_sub[index]=tau[9]
            autocorr_A_main[index]=tau[10]
            autocorr_A_sub[index]=tau[11]
            autocorr_a_bkg[index]=tau[12]
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    out_file.write('a_bkg\t%.4e\t%.4e\t%.4e\n' % (results[12,0], results[12,1], results[12,2]))
    ...
    alpha_main_mcmc = results[6,0]
    beta_main_mcmc = results[7,0]
    gamma_main_mcmc = results[8,0]
    beta_sub_mcmc = results[9,0]
    A_main_mcmc = results[10,0]
    A_sub_mcmc = results[11,0]
    a_bkg_mcmc = results[12,0]
    ...
    mod_main = models.GNFW_model_cluster_circ(x, y, x_main_mcmc, y_main_mcmc, rs_main_mcmc, beta_main_mcmc, alpha_main_mcmc, gamma_main_mcmc, A_main_mcmc)
    ...
    fig = corner.corner(postsamples[:, :], labels=[r'$x_{main}$', r'$y_{main}$', r'$x_{sub}$', \
                                                   r'$y_{sub}$', r'$rs_{main}$', r'$rc_{sub}$', \
                                                   r'$\alpha_{main}$', r'$\beta_{main}$', \
                                                   r'$\gamma_{main}$', r'$\beta_{sub}$', \
    ...
    y_alpha_main = autocorr_alpha_main[:index]
    y_beta_main = autocorr_beta_main[:index]
    y_gamma_main = autocorr_gamma_main[:index]
    ...
    plt.plot(n, y_alpha_main, label=r'$\alpha_{main}$')
    plt.plot(n, y_beta_main, label=r'$\beta_{main}$')
    plt.plot(n, y_gamma_main, label=r'$\gamma_{main}$')
