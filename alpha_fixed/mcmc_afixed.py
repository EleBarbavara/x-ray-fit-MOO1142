...
def log_likelihood (var):            
    ...
    if fit_method == 'gnfw_circ+beta_circ':      
        ...
        mod_main = models.GNFW_model_cluster_circ(x, y, x_main_fix, y_main_fix, rs_main, beta_main, alpha_main_fix, gamma_main, A_main)
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    x_sub_in, y_sub_in, rs_main_in, rc_sub_in, beta_main_in, gamma_main_in, beta_sub_in, \
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    init_sample = np.array ([x_sub_first, y_sub_first, rs_main_first, rc_sub_first, \
                            beta_main_first, gamma_main_first, beta_sub_first, A_main_first, A_sub_first, a_bkg_first]).T  #x_main_first, y_main_first, 
...
with (Pool(ncores)) as pool:
    ...
    for k in range(N_check):
        ...
        if fit_method == 'gnfw_circ+beta_circ':
            ...
            autocorr_gamma_main[index]=tau[5]
            autocorr_beta_sub[index]=tau[6]
            autocorr_A_main[index]=tau[7]
            autocorr_A_sub[index]=tau[8]
            autocorr_a_bkg[index]=tau[9]
...
if fit_method == 'gnfw_circ+beta_circ':
    ...
    out_file.write('alpha_main\t%.4e\n' % alpha_main_fix)
    out_file.write('beta_main\t%.4e\t%.4e\t%.4e\n' % (results[4,0]/2, results[4,1], results[4,2]))
    out_file.write('gamma_main\t%.4e\t%.4e\t%.4e\n' % (results[5,0]/2, results[5,1], results[5,2]))
    ...
    gamma_main_mcmc = results[5,0]
    beta_sub_mcmc = results[6,0]
    A_main_mcmc = results[7,0]
    A_sub_mcmc = results[8,0]
    a_bkg_mcmc = results[9,0]
    ...
    gamma_main_mcmc = results[7,0]
    beta_sub_mcmc = results[8,0]
    A_main_mcmc = results[9,0]
    A_sub_mcmc = results[10,0]
    a_bkg_mcmc = results[11,0]
    ...
    mod_main = models.GNFW_model_cluster_circ(x, y, x_main_fix, y_main_fix, rs_main_mcmc, beta_main_mcmc, alpha_main_fix, gamma_main_mcmc, A_main_mcmc)
    ...
    fig = corner.corner(postsamples[:, :], labels=[r'$x_{sub}$', r'$y_{sub}$', r'$rs_{main}$', \
                                                   r'$rc_{sub}$', r'$\beta_{main}$', \
